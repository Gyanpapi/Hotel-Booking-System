  ...
  if(isset($_GET['id']))
  {
	...
	if(isset($_POST['submit']))
    {
       if(empty($_POST['email'])  OR empty($_POST['full_name']) OR empty($_POST['phone_number']) OR empty($_POST['check_in'])  OR empty($_POST['check_out']))
	   {
        ...
        $_SESSION['price']=$singleRoom->price *intval($diff->format('%R%a'));
		$sess=$_SESSION['price'];
	    		...
	    		<div class="col-lg-6">
					<form action="room-single.php?id=<?php echo $id;?>" method="POST" class="appointment-form" style="margin-top: -568px;">
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="form-group">
								<input type="text" name="hotel_name" class="form-control" value="<?php echo $singleRoom->price."$ (per night)"; ?>" readonly>
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
